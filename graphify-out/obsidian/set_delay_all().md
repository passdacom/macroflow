---
source_file: "./src/macroflow/macro_file.py"
type: "code"
community: "test macro file.py load"
location: "L329"
tags:
  - graphify/code
  - graphify/INFERRED
  - community/test_macro_file.py_load
---

# set_delay_all()

## Connections
- [[._set_delay_all()]] - `calls` [INFERRED]
- [[MacroData]] - `calls` [INFERRED]
- [[macro_file.py]] - `contains` [EXTRACTED]
- [[test_set_delay_all()]] - `calls` [INFERRED]
- [[test_set_delay_all_can_target_only_displayed_primary_events()]] - `calls` [INFERRED]
- [[test_set_delay_single_to_none()]] - `calls` [INFERRED]
- [[대상 events의 재생 대기를 동일 값 또는 녹화 타이밍(None)으로 설정한다.      Args         macro 원본 Macr]] - `rationale_for` [EXTRACTED]

#graphify/code #graphify/INFERRED #community/test_macro_file.py_load