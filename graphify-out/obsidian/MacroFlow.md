...
## Connections
- [[.. 가 섞인 상대 경로도 같은 실제 파일 경로로 정규화해야 한다.]] - `uses` [INFERRED]
- [[._build_flow()]] - `calls` [INFERRED]
...
- [[test_iter_linear_macro_paths_normalizes_relative_dot_segments()]] - `calls` [INFERRED]
- [[test_linear_flow_reports_saved_uniform_gap()]] - `calls` [INFERRED]
...
- [[같은 파일은 절대상대 dot-segment 표기가 달라도 한 번만 추가해야 한다.]] - `uses` [INFERRED]
...
- [[대기종료 노드는 매크로 목록 행 상태를 덮어쓰면 안 된다.]] - `uses` [INFERRED]
- [[매크로 사이 대기 노드가 있어도 모든 MacroNode 경로를 순서대로 추출한다.]] - `uses` [INFERRED]
...
- [[시퀀스 실제 대기와 병합 timestamp 간격의 speed 의미를 구분한다.]] - `uses` [INFERRED]
