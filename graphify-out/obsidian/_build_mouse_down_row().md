---
source_file: "/root/.openclaw/workspace/macroflow/src/macroflow/ui/editor_rows.py"
type: "code"
community: "build rows editor rows.py"
location: "L147"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/build_rows_editor_rows.py
---

# _build_mouse_down_row()

## Connections
- [[_DisplayRow]] - `calls` [EXTRACTED]
- [[_build_rows()]] - `calls` [EXTRACTED]
- [[_button_label()]] - `calls` [EXTRACTED]
- [[_color_check_emoji()]] - `calls` [EXTRACTED]
- [[_color_check_kind()]] - `calls` [EXTRACTED]
- [[_delay_str()]] - `calls` [EXTRACTED]
- [[_position_detail()]] - `calls` [EXTRACTED]
- [[_time_ms()]] - `calls` [EXTRACTED]
- [[editor_rows.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/build_rows_editor_rows.py