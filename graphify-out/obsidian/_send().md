---
source_file: "/root/.openclaw/workspace/macroflow/src/macroflow/win32/sendinput.py"
type: "code"
community: "execute event PlayState test"
location: "L128"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/execute_event_PlayState_test
---

# _send()

## Connections
- [[send_key()_1]] - `calls` [EXTRACTED]
- [[send_mouse_button()_1]] - `calls` [EXTRACTED]
- [[send_mouse_click()_1]] - `calls` [EXTRACTED]
- [[send_mouse_drag()_1]] - `calls` [EXTRACTED]
- [[send_mouse_move()_1]] - `calls` [EXTRACTED]
- [[send_mouse_wheel()_1]] - `calls` [EXTRACTED]
- [[send_text()_1]] - `calls` [EXTRACTED]
- [[sendinput.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/execute_event_PlayState_test