...
## Corpus Check
- 101 files · ~83,242 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1365 nodes · 5059 edges · 37 communities detected
- Extraction: 39% EXTRACTED · 61% INFERRED · 0% AMBIGUOUS · INFERRED: 3081 edges (avg confidence: 0.58)
...
Cohesion: 0.07
Nodes (224): Pure event insertion helpers for the MacroFlow event editor.  This module intent, Return events with click or double-click MouseButtonEvents inserted., Return events with one configured ColorTriggerEvent inserted., Return the source event index after which an editor insertion should occur., Insert events after an index and shift following timestamps to preserve timing., Return events with one TextInputEvent inserted and later timestamps shifted., MacroFlow 이벤트 에디터 위젯.  그룹 표시: mouse_down+up → 클릭, key_down+up → 키 입력. Undo/Redo,, 지정 행 클릭 이벤트의 색 체크(color_check_enabled)를 토글한다.          recorded_color가 없는 이벤트에서는 (+216 more)

### Community 1 - "MainWindow OverlayWindow RepeatPlaybackSession PlaybackStartOptions"
Cohesion: 0.03
Nodes (81): Exception, 재생 중 ESC×3 긴급 중지 감지용 키보드 Hook을 시작한다.      LLKHF_INJECTED 이벤트(SendInput 주입)는 무시하므, 긴급 중지 Hook을 해제하고 스레드를 종료한다., start_emergency_hook(), stop_emergency_hook(), MainWindow, MacroFlow 메인 창.  전체 상태 머신(idle / recording / stopping / playing)을 관리한다. F6/F7 글로, 시퀀스 완료/오류 시 emergency hook 해제 후 툴바·상태바를 갱신한다. (+73 more)

### Community 2 - "MacroSequencerWidget EndNode MacroFlow FlowEngine"
Cohesion: 0.04
Nodes (86): _run_sequencer_smoke(), _key(), _macro(), main(), _meta(), _move(), parse_args(), _rewrite_flow_with_dot_segments() (+78 more)

### Community 3 - "EventEditorWidget add single row"
Cohesion: 0.04
Nodes (56): create_capture_controls(), create_delay_spin(), create_percentage_spin(), Small widget factories shared by MacroFlow editor dialogs., Create a position percentage spin box with the editor's shared bounds., Create a millisecond delay spin box with the editor's shared suffix., Create the standard F6 capture label/button pair used in editor dialogs., EventEditorWidget (+48 more)

### Community 4 - "execute event PlayState test"
Cohesion: 0.04
Nodes (74): find_window(), get_pixel_color(), GetPixel로 단일 픽셀 RGB 값을 반환한다.      core-beliefs.md 원칙 7: 스크린샷 API 금지 — GetPixel만, title_contains를 제목에 포함하는 창 핸들(HWND)을 반환한다.      Args:         title_contains: 검색, _color_matches(), _event_timing_compensation_ns(), _execute_event(), _execute_event_sequence() (+66 more)
...
### Community 9 - "test macro file.py load"
Cohesion: 0.11
...
### Community 10 - "test editor table.py table"
Cohesion: 0.08
...
Cohesion: 0.1
Nodes (10): _check_esc_triple(), _consumer_loop(), _convert_raw(), get_event_count(), is_recording(), stop_recording(), _vk_to_key(), TestConvertRaw (+2 more)
...
### Community 13 - "test editor insertions.py insert"
Cohesion: 0.22
...
### Community 14 - "test color settings regressions.py"
Cohesion: 0.12
...
### Community 15 - "rdp runtime safety smoke.py"
Cohesion: 0.2
Nodes (15): evaluate_hotkey_result(), evaluate_sequencer_result(), format_status_line(), main(), parse_args(), _press_function_key(), _pump_until(), _run_hotkey_smoke() (+7 more)

### Community 16 - "append recording test append"
Cohesion: 0.27
...
### Community 17 - "test editor context menu.py"
Cohesion: 0.4
...
## Knowledge Gaps
- **166 isolated node(s):** `MacroFlow 진입점.  실행 즉시 파일 로그를 시작하고, PyQt6 메인 창을 표시한다. PyQt6 로드 실패 시 ctypes Messag`, `파일 로그 핸들러를 설정하고 로그 파일 경로를 반환한다.`, `Win32 MessageBoxW로 치명적 오류를 표시한다 (PyQt6 폴백).      Args:         title: 다이얼로그 제목.`, `Pure validation rules for MacroFlow condition expressions.  This module owns the`, `Return whether an AST node is statically a numeric expression.` (+161 more)
...
- **Why does `MacroData` connect `MacroData MouseButtonEvent KeyEvent MacroSettings` to `MainWindow OverlayWindow RepeatPlaybackSession PlaybackStartOptions`, `MacroSequencerWidget EndNode MacroFlow FlowEngine`, `EventEditorWidget add single row`, `execute event PlayState test`, `test macro file.py load`, `convert raw TestConvertRaw stop`, `append recording test append`, `test editor context menu.py`?**
  _High betweenness centrality (0.233) - this node is a cross-community bridge._
- **Why does `EventEditorWidget` connect `EventEditorWidget add single row` to `MacroData MouseButtonEvent KeyEvent MacroSettings`, `MainWindow OverlayWindow RepeatPlaybackSession PlaybackStartOptions`?**
  _High betweenness centrality (0.141) - this node is a cross-community bridge._
- **Why does `MouseButtonEvent` connect `MacroData MouseButtonEvent KeyEvent MacroSettings` to `EventEditorWidget add single row`, `execute event PlayState test`, `TestTargetApp test target app.py`, `build rows editor rows.py`, `test macro file.py load`, `convert raw TestConvertRaw stop`, `test color settings regressions.py`, `rdp runtime safety smoke.py`, `test editor context menu.py`?**
  _High betweenness centrality (0.129) - this node is a cross-community bridge._
- **Are the 248 inferred relationships involving `MacroData` (e.g. with `PlaybackError` and `_PlayState`) actually correct?**
  _`MacroData` has 248 INFERRED edges - model-reasoned connections that need verification._
