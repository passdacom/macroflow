...
### 시퀀서 저장/로드 및 즐겨찾기 탭 동작 개선

#### ① BUG-01: 시퀀서 저장 후 다중 매크로 불러오기 실패 수정 (`script_engine.py`, `sequencer.py`)
- **현상**: 시퀀서에 매크로를 2개 이상 등록하고 `.macroflow`로 저장한 뒤 다시 열면 첫 번째 매크로만 목록에 복원됨
- **원인**: 시퀀서 저장 로직은 매크로 사이 간격(`gap_ms`)이 0보다 크면 `MacroNode → WaitFixedNode → MacroNode` 형태로 저장한다. 기존 로드 로직은 `MacroNode`만 따라가고, `WaitFixedNode`를 만나면 순회를 중단해서 첫 번째 매크로만 남았다.
- **수정**:
  - `script_engine.py`에 `iter_linear_macro_paths(flow, flow_path)` 추가
  - `MacroNode`는 경로를 수집하고 `next_on_success`로 이동
  - `WaitFixedNode`는 저장된 `next`로 이동해 다음 매크로까지 계속 순회
  - `EndNode` 또는 비선형 노드를 만나면 안전하게 종료
  - 절대 경로는 그대로 유지하고, 상대 경로는 `.macroflow` 파일 위치 기준으로 복원
- **효과**: 간격 대기 노드가 포함된 선형 시퀀스도 저장 후 다시 열 때 모든 매크로가 순서대로 복원된다.

#### ② BUG-02: 시퀀서 저장 버튼 분리 (`sequencer.py`, `main_window.py`)
- **현상**: 시퀀서의 저장 동작이 사실상 "다른 이름으로 저장" 중심으로 동작해, 이미 열린 `.macroflow` 파일을 명확하게 덮어쓰기 어렵다.
- **수정**:
  - 시퀀서 툴바에 `저장 Ctrl+S`와 `다른 이름으로 저장` 액션을 분리
  - 저장 대상 파일(`_current_flow_path`)이 있으면 덮어쓰기 확인 다이얼로그 표시 후 저장
  - 저장 대상 파일이 없으면 `다른 이름으로 저장`으로 위임
  - 메인 창 `Ctrl+S`와 `다른 이름으로 저장` 메뉴도 현재 탭이 시퀀서이면 `MacroSequencerWidget.save_flow()` / `save_flow_as()`로 라우팅
- **효과**: 에디터 저장 UX와 시퀀서 저장 UX가 일관되고, 사용자가 덮어쓰기와 신규 저장을 명확히 구분할 수 있다.

#### ③ FEAT-01: 즐겨찾기 탭에서 녹화·재생 비활성화 (`main_window.py`)
- **현상**: 즐겨찾기 탭은 직접 녹화·재생할 대상이 없는데 F7 또는 재생 버튼이 현재 에디터 매크로를 실행할 수 있었다.
- **수정**:
  - 즐겨찾기 탭에서는 F6 녹화뿐 아니라 F7 재생도 무시
  - `_toggle_playback()`에서도 즐겨찾기 탭이면 즉시 반환하도록 방어
  - `_update_toolbar()`에서 즐겨찾기 탭의 재생 버튼을 비활성화
- **효과**: 즐겨찾기 관리 중 실수로 기존 매크로가 재생되는 동작을 차단한다.

#### ④ FEAT-02: 즐겨찾기 → 시퀀서 추가 시 탭 자동 전환 제거 (`main_window.py`)
- **현상**: 즐겨찾기 우클릭 → "시퀀서에 추가"를 실행하면 즉시 시퀀서 탭으로 이동해, 여러 항목을 연속 추가할 때 흐름이 끊겼다.
- **원인**: 실제 탭 전환은 `favorites.py`가 아니라 `MainWindow._add_favorite_to_sequencer()` 신호 핸들러에서 수행되고 있었다.
- **수정**: `self._tabs.setCurrentWidget(self._sequencer)` 호출 제거, 상태바 메시지만 갱신
- **효과**: 즐겨찾기 탭에 머무른 채 여러 매크로를 연속으로 시퀀서에 추가할 수 있다.

#### ⑤ 회귀 테스트 추가 (`tests/test_sequencer_backlog.py`)
- `WaitFixedNode`가 포함된 선형 플로우에서 모든 `MacroNode` 경로가 추출되는지 검증
- 시퀀서 툴바에 저장/다른 이름 저장 액션이 분리되어 있는지 검증
- 메인 창 저장 단축키가 시퀀서 탭에서 플로우 저장으로 위임되는지 검증
- 즐겨찾기 → 시퀀서 추가 시 탭 전환 코드가 제거되었는지 검증
- 즐겨찾기 탭에서 재생 버튼이 비활성화되는지 검증

#### 검증
- `.venv/bin/pytest -q` → 64개 통과
- `.venv/bin/ruff check .` → 오류 없음
- `git diff --check` → 공백 오류 없음
- 커밋: `8840e41 fix sequencer save and favorites tab behavior`

#### 후속 제안
- 시퀀서 목록 변경 시 dirty 상태 표시(`*`)와 저장 전 닫기/불러오기 확인 다이얼로그 추가
- `.macroflow` 로드 시 비선형 플로우나 지원하지 않는 노드를 만나면 "부분 로드" 경고를 보여주기
- PyQt 의존 없이 시퀀서 저장/로드 로직을 더 많이 단위 테스트할 수 있도록 순수 헬퍼를 추가 분리
- 즐겨찾기에서 시퀀서에 추가한 항목 수를 상태바에 누적 표시하거나 토스트로 표시해 연속 추가 피드백 강화
...
#### ① drag sleep 제거 — 타이밍 드리프트 수정 (`win32/sendinput.py`)
- **원인**: `send_mouse_drag()` 보간 루프에서 `time.sleep(0.01) × 10` = 100ms 강제 블로킹
  → player.py 절대 타임스탬프 보정 루프 바깥에서 발생하므로 이후 이벤트 전체가 ~100ms 지연됨
  → core-beliefs.md 원칙 3 직접 위반
- **수정**: sleep 제거, 보간 이동 포함 전체 입력을 단일 `_send(*inputs)` 배치 호출로 대체
- **추가**: 이제 `time` 임포트 불필요 → 제거

#### ② GDI 핸들 누수 수정 (`win32/hooks.py`)
- **원인**: `get_pixel_color()`에서 예외 발생 시 `ReleaseDC` 미호출 → GDI 핸들 누수
  → 색 트리거 폴링 루프에서 반복 호출 시 장시간 후 GDI 핸들 고갈 가능
- **수정**: `try/finally` 블록으로 `ReleaseDC` 항상 실행 보장

---

## v0.2.5 — 2026-04-14

### 버그 수정: 음수 딜레이 이벤트 순서 역전 방지 (`player.py`)

#### 문제
음수 `delay_override_ms`를 설정하면 `target_ns = last_event_end_ns + (음수값)` 계산으로
직전 유의미한 이벤트(클릭·키 입력 등)보다 앞선 시각에 이벤트가 실행될 수 있었다.
예) `좌클릭 → 마우스이동 → 우클릭(딜레이=-10000ms)` 에서 우클릭이 좌클릭보다 먼저 실행될 수 있음.

#### 수정
- `last_significant_event_end_ns` 추적 변수 추가 — `MouseMoveEvent` 를 제외한 이벤트 완료 시마다 갱신
- `delay_override_ms`로 `target_ns` 계산 시, 마우스 이동이 아닌 이벤트는 `max(target_ns, last_significant_event_end_ns)` 클램프
- 결과: 음수 딜레이는 간격을 줄여 "즉시 실행"까지만 당길 수 있고, 이전 이벤트보다 앞설 수 없음
- 마우스 이동은 클램프 제외 (순서 제약 불필요)
...
#### ① 색 체크 삽입 타임스탬프 오류 (`ui/editor.py`)
- **원인**: `time.perf_counter_ns()` 절대값을 타임스탬프로 사용 → 매크로 상대 ts 범위(수백ms)와 동떨어진 수백억ns 값이 삽입됨
- **수정**: 직전 이벤트 `timestamp_ns + 1_000_000_000` (1초)으로 설정
- **추가**: 삽입 이후 이벤트들을 동일하게 1초 시프트 → 상대 타이밍 보존 (`dataclasses.replace()` 사용)

#### ② 색/창 트리거 이후 급속 재생 (`player.py`)
- **원인**: color/window 트리거가 예상보다 오래 실행되면 이후 이벤트의 `target_ns`가 이미 과거가 되어 연속 즉시 실행됨
- **수정**: 트리거 실행 후 `play_start_ns += trigger_duration_ns` 보정 → 이후 모든 이벤트가 녹화 시 상대 간격 유지

#### ③ 딜레이 음수 입력 지원 (`ui/editor.py`)
- 개별 딜레이/일괄 딜레이 모두 범위를 `-60000~60000ms`로 확장
- 음수 딜레이 = `target_ns < last_event_end_ns` → sleep 없이 즉시 실행 → 이전 이벤트보다 빠르게 실행 가능

#### ④ 우측 키패드 `vk_0x61` 표시 수정 (`recorder.py`)
- **원인**: `_VK_NAMES`에 숫자패드 VK 코드(0x60~0x6F) 미등록 → fallback으로 `vk_0x61` 형식 출력
- **수정**: `num0`~`num9`, `num*`, `num+`, `num-`, `num.`, `num/` 추가
- OEM 키 이름을 editor.py 기호(`".", ","` 등)로 통일
- editor.py에 구버전 이름 하위 호환 별칭 추가 (`"period"→0xBE` 등)
