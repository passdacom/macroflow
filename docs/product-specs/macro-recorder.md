...
| 동작 | 기본 단축키 | 비고 |
|---|---|---|
| 녹화 시작/중지 또는 위치 캡처 | F6 | 글로벌, 설정에서 F1~F24 중 변경 가능(F12 제외) |
| 재생 시작/중지 또는 색상 캡처 | F7 | 글로벌, 설정에서 변경 가능 |
| 녹화/재생 일시중지·재개 | F8 | 글로벌, 설정에서 변경 가능 |
| 빠른 텍스트 입력 기록 | F9 | 글로벌, 설정에서 변경 가능 |
| 텍스트 입력 동작 추가 | Ctrl+Shift+T | 매크로 에디터 내부, 비워둘 수 있음 |
| 클릭 동작 추가 | Ctrl+Shift+L | 매크로 에디터 내부, 비워둘 수 있음 |
| 색 체크 동작 삽입 | Ctrl+Shift+G | 매크로 에디터 내부, 비워둘 수 있음 |
| 긴급 전체 중지 | ESC × 3회 (500ms 이내) | 변경 불가 고정 |

### 단축키 충돌 방지 전략
- F6/F7/F8/F9는 기존 사용자 경험을 유지하기 위한 기본값
- 앱 내부 중복·예약 조합·고정 파일/편집 단축키 충돌은 적용 전에 검증
- 다른 프로그램의 글로벌 키 점유는 적용 시 실제 Windows `RegisterHotKey` 결과로 판정
- 새 글로벌 세트 등록이 실패하면 부분 등록을 정리하고 이전 정상 세트 전체를 복구
- 활성 글로벌 단축키의 key_down/key_up은 raw_events에 기록하지 않음
...
### 임시 저장 시점
- 녹화 worker와 Hook이 정상 종료된 직후 `.json`으로 자동 저장
- 최근 녹화 10개만 유지하고 더 오래된 파일은 삭제
- 현재 버전은 녹화 중 주기적 snapshot을 만들지 않으므로, 녹화 도중 앱이나 PC가
  비정상 종료되면 해당 세션은 복구할 수 없음

### 최근 녹화 복구
- 앱의 "최근 녹화" 메뉴에서 정상 종료·저장된 최근 10개 `.json` 파일을 선택해 복구
- 현재 버전은 재시작 시 자동 복구 알림을 표시하지 않음
