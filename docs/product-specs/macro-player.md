...
### 숨겨진 마우스 이동과 재생 대기
- 에디터의 `이동 표시`는 `mouse_move`를 표에서만 숨기며 재생 데이터에서 제거하지 않는다.
- 다음 표시 동작의 `delay_override_ms=0`은 숨겨진 이동을 건너뛰지 않는다. 이동은 녹화
  타이밍대로 실행되고 다음 동작은 마지막 실제 이동 직후 실행된다.
- 마우스 이동을 재생에서 제외하려면 `이동 삭제`로 events에서 제거해야 한다.
- 예를 들어 표시 동작 기록 시점이 500ms와 10000ms라면 녹화 간격은 9500ms다.
  두 번째 동작을 0ms로 설정해도 사이의 숨겨진 이동은 유지되며, 이동이 삭제된 경우에만
  두 번째 동작이 첫 번째 동작 직후 실행된다.

### 시퀀서 대기의 에디터 병합
- 시퀀서에서 실행하는 고정 `대기` 단계는 재생 속도와 무관한 orchestration wait다.
- 혼합 시퀀스를 에디터로 병합하면 이 단계는 단일 매크로의 `WaitEvent`로 변환되므로
  일반 매크로 타임라인과 같이 재생 속도의 영향을 받는다.
