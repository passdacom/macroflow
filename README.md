...
Windows 배포본은 매크로와 즐겨찾기를 버전별 실행 폴더가 아니라 `%APPDATA%\MacroFlow\` 아래에 보관합니다. 따라서 새 버전을 다른 폴더에 압축 해제하거나 설치해도 같은 사용자 데이터를 불러옵니다.
...
4. 같은 이름의 다른 파일이 이미 있으면 덮어쓰지 않고 `.legacy-<hash>` 접미사로 양쪽을 보존합니다.
5. 복사나 설정 저장이 실패하면 기존 EXE 옆 데이터로 fail-closed fallback합니다.
