...
- 창 최소 폭 제한을 제거하고 일반 작업에 맞는 1180px 초기 폭만 제공
- 반복·반복 간격·구간 입력 폭을 확대하고 기록 간격을 동작 추가 행으로 이동
- 시퀀서 단계 추가 버튼 이름을 명확히 하고 에디터의 모든 열을 수동 조절 가능하게 변경

### 기능

- 매크로 파일과 문구·클릭·색상 대기·대기 단계가 섞인 시퀀스를 에디터로 병합
- 재생 대기와 숨겨진 마우스 이동의 실제 동작을 도움말 메뉴에 추가

### 안정성

- 혼합 병합의 중복 단계·이벤트 ID와 재생 설정 충돌을 병합 전에 거부하거나 안전하게 보정
- 단순 시퀀서 대기값을 `0..30000ms` 정수로 통일하고 Wait-only flow의 저장·재로드 보장
- 일반 MacroFlow의 비음수 장기 대기 호환을 유지하면서 음수 대기를 strict load에서 거부
...
- 탭 전환 시 런타임·재생 설정 두 행을 숨기지 않고 기능 종속성에 따라 활성화 상태만 변경
- 속도·반복·반복 간격과 구간 재생을 같은 행에 고정하고 최소 창 너비를 1280px로 확대
- 매크로·플로우 파일 작업을 각 탭의 동일한 첫 행에 배치
- 에디터 이동·대기·내보내기·복구·기록 간격 작업을 한 행으로 통합
