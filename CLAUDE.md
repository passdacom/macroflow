...
### v1.0.0 추가 구현 (2026-04-16)
- [x] 시퀀서 속도 설정 반영 버그 수정 (FlowEngine speed 파라미터)
- [x] 클릭 시 픽셀 색 자동 저장 (recorded_color) — 녹화 시 GetPixel 자동 호출
- [x] 클릭 이벤트 색 체크 기능 — 에디터 우클릭 → "🎨 색 체크 켜기/끄기"
- [x] 색 체크 활성 클릭은 에디터에서 주황색으로 시각 구분
- [x] 재생 시 색 체크 불일치 클릭 자동 스킵
- [x] 모든 색 체크 실행 전 마우스 이동 선행 (hover 효과 트리거)
- [x] 시퀀스 완료 후 상태바 "대기 중" 복원
- [x] 시퀀서 실행 중 속도/반복/간격 설정 비활성화
- [x] QSettings로 창 크기·위치와 마지막 파일 저장/자동 복원
- [x] 탭 전환 시 상태바 힌트 동적 변경
- [x] 버전 1.0.0

### v1.0.0 이후 추가 구현
- [x] 즐겨찾기 트리 구조 — 그룹별 분류, 아코디언 확장/축소, 드래그앤드롭 재정렬·그룹 이동
  - _index.json 기반 영구 저장, 고아 파일 자동 기본 그룹 편입
  - 우클릭: 에디터/시퀀서/그룹이동/제거, 그룹 우클릭: 이름변경/삭제/모두펼치기/접기

**현재 단계**: v1.0.0 패키징 완료. Phase2 기능(UI Automation, 서브플로우, 변수 시스템)은 다음 마일스톤.
